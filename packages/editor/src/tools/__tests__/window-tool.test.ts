import { describe, it, expect, beforeEach, vi } from 'vitest'
import { WindowTool } from '../window-tool'
import type { ToolContext, ToolPointerEvent } from '../types'
import type { CommandDispatcher } from '../../commands/dispatcher'

function makeEvent(overrides: Partial<ToolPointerEvent> = {}): ToolPointerEvent {
  return {
    x: 0,
    y: 0,
    lng: 0,
    lat: 0,
    button: 0,
    shiftKey: false,
    ctrlKey: false,
    altKey: false,
    ...overrides,
  }
}

function makeCtx(dispatcher?: { execute: ReturnType<typeof vi.fn> }): ToolContext {
  return {
    services: {
      dispatcher: (dispatcher ?? { execute: vi.fn() }) as unknown as CommandDispatcher,
    } as ToolContext['services'],
  }
}

describe('WindowTool', () => {
  let tool: WindowTool

  beforeEach(() => {
    tool = new WindowTool()
  })

  it('has correct id and label', () => {
    expect(tool.id).toBe('window')
    expect(tool.label).toBe('Window')
  })

  it('starts in inactive state', () => {
    expect(tool.state.placing).toBe(false)
    expect(tool.state.position).toBeNull()
  })

  describe('pointer flow', () => {
    it('begins placing on pointer down', () => {
      const ctx = makeCtx()
      tool.onPointerDown(makeEvent({ x: 1, y: 2 }), ctx)

      expect(tool.state.placing).toBe(true)
      expect(tool.state.position).toEqual({ x: 1, y: 2 })
    })

    it('updates position on pointer move while placing', () => {
      const ctx = makeCtx()
      tool.onPointerDown(makeEvent({ x: 0, y: 0 }), ctx)
      tool.onPointerMove(makeEvent({ x: 5, y: 10 }), ctx)

      expect(tool.state.position).toEqual({ x: 5, y: 10 })
    })

    it('ignores pointer move when not placing', () => {
      const ctx = makeCtx()
      tool.onPointerMove(makeEvent({ x: 5, y: 10 }), ctx)

      expect(tool.state.placing).toBe(false)
      expect(tool.state.position).toBeNull()
    })

    it('dispatches window.create on pointer up', () => {
      const execute = vi.fn()
      const ctx = makeCtx({ execute })

      tool.onPointerDown(makeEvent({ x: 1, y: 2 }), ctx)
      tool.onPointerUp(makeEvent({ x: 5, y: 8 }), ctx)

      expect(execute).toHaveBeenCalledOnce()
      expect(execute).toHaveBeenCalledWith({
        id: 'window.create',
        label: 'Create Window',
        payload: {
          buildingId: '',
          floorId: '',
          window: {
            wallId: '',
            offset: 0,
            width: 1.2,
            sillHeight: 0.9,
            metadata: {},
          },
        },
      })
    })

    it('resets state after pointer up', () => {
      const ctx = makeCtx()
      tool.onPointerDown(makeEvent({ x: 1, y: 2 }), ctx)
      tool.onPointerUp(makeEvent({ x: 5, y: 8 }), ctx)

      expect(tool.state.placing).toBe(false)
      expect(tool.state.position).toBeNull()
    })

    it('does not dispatch if pointer up without pointer down', () => {
      const execute = vi.fn()
      const ctx = makeCtx({ execute })

      tool.onPointerUp(makeEvent({ x: 5, y: 8 }), ctx)

      expect(execute).not.toHaveBeenCalled()
    })
  })

  describe('cancel', () => {
    it('resets state on Escape key', () => {
      const ctx = makeCtx()
      tool.onPointerDown(makeEvent({ x: 1, y: 2 }), ctx)
      tool.onKeyDown({ key: 'Escape' } as KeyboardEvent, ctx)

      expect(tool.state.placing).toBe(false)
      expect(tool.state.position).toBeNull()
    })

    it('does not dispatch on cancel', () => {
      const execute = vi.fn()
      const ctx = makeCtx({ execute })

      tool.onPointerDown(makeEvent({ x: 1, y: 2 }), ctx)
      tool.onKeyDown({ key: 'Escape' } as KeyboardEvent, ctx)

      expect(execute).not.toHaveBeenCalled()
    })
  })

  describe('activation lifecycle', () => {
    it('resets state on activate', () => {
      const ctx = makeCtx()
      tool.onPointerDown(makeEvent({ x: 1, y: 2 }), ctx)
      expect(tool.state.placing).toBe(true)

      tool.onActivate(ctx)
      expect(tool.state.placing).toBe(false)
    })

    it('resets state on deactivate', () => {
      const ctx = makeCtx()
      tool.onPointerDown(makeEvent({ x: 1, y: 2 }), ctx)
      expect(tool.state.placing).toBe(true)

      tool.onDeactivate(ctx)
      expect(tool.state.placing).toBe(false)
    })
  })

  describe('window data contract', () => {
    it('dispatches window with correct SPEC fields', () => {
      const execute = vi.fn()
      const ctx = makeCtx({ execute })

      tool.onPointerDown(makeEvent({ x: 0, y: 0 }), ctx)
      tool.onPointerUp(makeEvent({ x: 10, y: 5 }), ctx)

      const command = execute.mock.calls[0][0]
      const payload = command.payload

      expect(payload).toHaveProperty('buildingId')
      expect(payload).toHaveProperty('floorId')
      expect(payload).toHaveProperty('window')

      const win = payload.window
      expect(win).toHaveProperty('wallId')
      expect(win).toHaveProperty('offset')
      expect(win).toHaveProperty('width')
      expect(win).toHaveProperty('sillHeight')
      expect(win).toHaveProperty('metadata')

      expect(typeof win.wallId).toBe('string')
      expect(typeof win.offset).toBe('number')
      expect(typeof win.width).toBe('number')
      expect(typeof win.sillHeight).toBe('number')
      expect(typeof win.metadata).toBe('object')
    })

    it('uses default width 1.2', () => {
      const execute = vi.fn()
      const ctx = makeCtx({ execute })

      tool.onPointerDown(makeEvent({ x: 0, y: 0 }), ctx)
      tool.onPointerUp(makeEvent({ x: 1, y: 1 }), ctx)

      expect(execute.mock.calls[0][0].payload.window.width).toBe(1.2)
    })

    it('uses default sillHeight 0.9', () => {
      const execute = vi.fn()
      const ctx = makeCtx({ execute })

      tool.onPointerDown(makeEvent({ x: 0, y: 0 }), ctx)
      tool.onPointerUp(makeEvent({ x: 1, y: 1 }), ctx)

      expect(execute.mock.calls[0][0].payload.window.sillHeight).toBe(0.9)
    })
  })
})
