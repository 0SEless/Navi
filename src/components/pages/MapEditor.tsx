...
import {
  ...
  Move, Building2, MapPin, Route, Upload, Info, Camera, QrCode, X, Layers,
} from "lucide-react";
import { RealMapView } from "@/components/map-editor/RealMapView";
import { AutoSvgView } from "@/components/map-editor/AutoSvgView";
import {
  MOCK_BUILDINGS, MOCK_NODES, MOCK_EDGES,
  CAMPUS_CENTER, NODE_COLORS, generateId, calcDistance,
} from "@/components/map-editor/mockData";
import type {
...
export function MapEditor() {
  ...
  const handleFloorPlanUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const selectedBuildingData = selectedBuildingId ? graph.getBuilding(selectedBuildingId) : null;
  ...
  const handleUpdateBuilding = useCallback((field: string, value: unknown) => {
    if (selectedBuildingId) {
      graph.updateBuilding(selectedBuildingId, { [field]: value });
    }
  }, [selectedBuildingId, graph]);
        ...
        <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
          {view === "real" && (
            <RealMapView
              ...
              onSelectNode={(id) => { setSelectedNode(id); setSelectedBuildingId(null); }}
              onSelectEdge={(id) => { setSelectedEdge(id); setSelectedBuildingId(null); }}
              onSelectBuilding={(id) => { setSelectedBuildingId(id); setSelectedNode(null); setSelectedEdge(null); }}
        ...
        >
          <div style={{ padding: "10px 14px", borderBottom: "1px solid #1E293B", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ color: "#94A3B8", fontSize: 11, fontWeight: 600 }}>
              {selectedBuildingData ? "Building Properties" : selectedNodeData ? "Node Properties" : selectedEdgeData ? "Edge Properties" : "Properties"}
            </span>
          </div>

          {selectedBuildingData ? (
            <div style={{ flex: 1, overflowY: "auto", padding: "14px" }}>
              <div style={{ background: "rgba(251,191,36,0.12)", border: "1px solid rgba(251,191,36,0.25)", borderRadius: 8, padding: "12px", marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 2, marginBottom: 4 }}>
                  <Building2 size={13} color="#FBBF24" />
                  <span style={{ color: "#FBBF24", fontSize: 13, fontWeight: 700, marginLeft: 4 }}>{selectedBuildingData.name}</span>
                  <span style={{ color: "#475569", fontSize: 10, marginLeft: 4 }}>{selectedBuildingData.id}</span>
                </div>
                <div style={{ display: "flex", gap: 2, marginTop: 4 }}>
                  {selectedBuildingData.code && <span style={{ background: "#1E293B", borderRadius: 3, padding: "1px 5px", fontSize: 10, color: "#94A3B8" }}>{selectedBuildingData.code}</span>}
                  <span style={{ background: "#1E293B", borderRadius: 3, padding: "1px 5px", fontSize: 10, color: "#94A3B8" }}>{selectedBuildingData.floors} floor{(selectedBuildingData.floors ?? 1) > 1 ? "s" : ""}</span>
                </div>
              </div>
              ...
              {/* Code */}
              <div style={{ marginBottom: 10 }}>
                <label style={{ color: "#475569", fontSize: 9, fontWeight: 600, letterSpacing: "0.08em", display: "block", marginBottom: 3 }}>CODE</label>
                <input
                  value={selectedBuildingData.code ?? ""}
                  onChange={(e) => handleUpdateBuilding("code", e.target.value.toUpperCase())}
                  style={{ width: "100%", background: "#111827", border: "1px solid #1E3A5F", borderRadius: 5, padding: "6px 8px", color: "#E2E8F0", fontSize: 11, outline: "none", boxSizing: "border-box" }}
                />
              </div>
              ...
              {/* Floors */}
              <div style={{ marginBottom: 10 }}>
                <label style={{ color: "#475569", fontSize: 9, fontWeight: 600, letterSpacing: "0.08em", display: "block", marginBottom: 3 }}>FLOORS</label>
                <input
                  type="number"
                  min={1}
                  max={20}
                  value={selectedBuildingData.floors ?? 1}
                  onChange={(e) => handleUpdateBuilding("floors", Math.max(1, parseInt(e.target.value) || 1))}
                  style={{ width: "100%", background: "#111827", border: "1px solid #1E3A5F", borderRadius: 5, padding: "6px 8px", color: "#E2E8F0", fontSize: 11, outline: "none", boxSizing: "border-box" }}
                />
              </div>
              ...
              {/* Color */}
              <div style={{ marginBottom: 10 }}>
                <label style={{ color: "#475569", fontSize: 9, fontWeight: 600, letterSpacing: "0.08em", display: "block", marginBottom: 3 }}>COLOR</label>
                <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                  <input
                    type="color"
                    value={selectedBuildingData.color ?? "#64748B"}
                  ...
                  <input
                    value={selectedBuildingData.color ?? "#64748B"}
                    onChange={(e) => handleUpdateBuilding("color", e.target.value)}
                    style={{ flex: 1, background: "#111827", border: "1px solid #1E3A5F", borderRadius: 5, padding: "6px 8px", color: "#E2E8F0", fontSize: 11, outline: "none", boxSizing: "border-box" }}
                  />
                </div>
              </div>
              ...
              {/* Description */}
              <div style={{ marginBottom: 10 }}>
                <label style={{ color: "#475569", fontSize: 9, fontWeight: 600, letterSpacing: "0.08em", display: "block", marginBottom: 3 }}>DESCRIPTION</label>
                <textarea
                  value={selectedBuildingData.description}
                  onChange={(e) => handleUpdateBuilding("description", e.target.value)}
                  rows={3}
                  style={{ width: "100%", background: "#111827", border: "1px solid #1E3A5F", borderRadius: 5, padding: "6px 8px", color: "#E2E8F0", fontSize: 11, outline: "none", boxSizing: "border-box", resize: "vertical", fontFamily: "inherit" }}
                />
              </div>

              {/* Floor Plan Upload */}
              <div style={{ marginBottom: 10, border: "1px solid #1E293B", borderRadius: 6, overflow: "hidden" }}>
                <div style={{ padding: "6px 10px", background: "#111827", borderBottom: "1px solid #1E293B", display: "flex", alignItems: "center", gap: 2 }}>
                  <Layers size={11} color="#94A3B8" />
                  <span style={{ color: "#94A3B8", fontSize: 10, fontWeight: 600, marginLeft: 4 }}>FLOOR PLANS</span>
                </div>
                <div style={{ padding: 10 }}>
                  {floorPlanFile ? (
                    <div style={{ background: "#0A0F1E", borderRadius: 5, padding: 8, border: "1px solid #1E3A5F" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                        <Upload size={12} color="#10B981" />
                        <span style={{ fontSize: 10, color: "#94A3B8", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{floorPlanFile.name}</span>
                        <span style={{ fontSize: 9, color: "#475569" }}>({(floorPlanFile.size / 1024).toFixed(0)} KB)</span>
                      </div>
                      <button
                        onClick={() => setFloorPlanFile(null)}
                        style={{ width: "100%", padding: "5px", background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 4, color: "#10B981", fontSize: 10, cursor: "pointer" }}
                      >
                        Replace file
                      </button>
                    </div>
                  ) : (
                    <label style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: "10px", border: "1px dashed #1E3A5F", borderRadius: 5, cursor: "pointer", background: "#0A0F1E" }}>
                      <Upload size={14} color="#475569" />
                      <span style={{ fontSize: 10, color: "#64748B" }}>Upload floor plan image</span>
                      <span style={{ fontSize: 9, color: "#475569" }}>PNG, JPG or SVG</span>
                      <input type="file" accept="image/png,image/jpeg,image/svg+xml" onChange={handleFloorPlanUpload} style={{ display: "none" }} />
                    </label>
                  )}
                </div>
              </div>
              ...
              {/* Building Nodes */}
              <div style={{ marginBottom: 10 }}>
                <label style={{ color: "#475569", fontSize: 9, fontWeight: 600, letterSpacing: "0.08em", display: "block", marginBottom: 4 }}>NODES ({graph.getNodesByBuilding(selectedBuildingData.id).length})</label>
                {graph.getNodesByBuilding(selectedBuildingData.id).slice(0, 5).map((n) => (
                  <div key={n.id} style={{ padding: "3px 6px", background: "#0A0F1E", border: "1px solid #1E293B", borderRadius: 3, marginBottom: 2, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ color: "#94A3B8", fontSize: 10 }}>{n.id} · {n.name}</span>
                    <span style={{ color: "#475569", fontSize: 9 }}>Floor {n.floor}</span>
                  </div>
                ))}
                {graph.getNodesByBuilding(selectedBuildingData.id).length > 5 && (
                  <div style={{ color: "#475569", fontSize: 9, textAlign: "center", padding: 4 }}>
                    +{graph.getNodesByBuilding(selectedBuildingData.id).length - 5} more
                  </div>
                )}
              </div>
              ...
              {/* Delete */}
              <button
                onClick={() => handleDeleteBuilding(selectedBuildingData.id)}
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (selectedEdge) graph.updateEdge(selectedEdge, { type: newType });
                  }}
                  style={{ width: "100%", background: "#111827", border: "1px solid #1E3A5F", borderRadius: 5, padding: "6px 8px", color: "#E2E8F0", fontSize: 11, outline: "none", cursor: "pointer" }}
                >
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, textAlign: "center" }}>
              <div style={{ width: 48, height: 48, background: "rgba(28,107,235,0.08)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 10 }}>
                <MousePointer2 size={20} color="#1C6BEB" style={{ opacity: 0.5 }} />
              </div>
              <p style={{ color: "#475569", fontSize: 11, lineHeight: 1.5 }}>Select a node, edge, or building on the map to edit its properties.</p>
            </div>
          )}
