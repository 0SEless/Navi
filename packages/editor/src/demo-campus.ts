...
import type {
  LatLng, LocalCoord, Building, Floor, Road, Panorama, QRCheckpoint, PanoramaHotspot,
} from '@navi/core'

let _cached: CampusDocument | null = null

export function getDemoCampus(): CampusDocument {
  if (_cached) return _cached
  _cached = buildCampus()
  return _cached
}

function $poly(pts: LatLng[]) { return { points: pts } }
function $lpoly(pts: LocalCoord[]) { return { points: pts } }
function $lpolyline(pts: LocalCoord[]) { return { points: pts } }
function $wpolyline(pts: LatLng[]) { return { points: pts } }
