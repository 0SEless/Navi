...
function buildNodeGeo(node: NavNode, selectedId: string | null): GeoJSON.Feature {
  return {
    type: "Feature",
    id: node.id,
    properties: {
      ...
      hasQr: node.hasQr ?? false,
      hasPanorama: node.hasPanorama ?? false,
      r: node.type === "intersection" ? 9 : 11,
    },
    geometry: { type: "Point", coordinates: [node.position.lng, node.position.lat] },
  };
}

function buildEdgeGeo(edge: NavEdge, selectedId: string | null, nodes: NavNode[]): GeoJSON.Feature | null {
...
export function RealMapView({
  ...
  onSelectNode, onSelectEdge, onSelectBuilding, onDeleteNode, onDeleteEdge,
  onPlaceComponent,
}: RealMapViewProps) {
  ...
  const cbsRef = useRef({ onAddNode, onAddEdge, onBuildingDrawn, onSelectNode, onSelectEdge, onSelectBuilding, onDeleteNode, onDeleteEdge, onPlaceComponent });
  ...
  useEffect(() => {
    ...
    cbsRef.current = { onAddNode, onAddEdge, onBuildingDrawn, onSelectNode, onSelectEdge, onSelectBuilding, onDeleteNode, onDeleteEdge, onPlaceComponent };
  });
  ...
  useEffect(() => {
    ...
    const handleClick = (e: maplibregl.MapMouseEvent) => {
      ...
      if (curTool === "select") {
        const hitBuilding = features.find((f) => (f.layer.id === LYR.BUILDINGS || f.layer.id === LYR.BUILDINGS_OUTLINE) && f.properties);
        if (hitNode) { cbs.onSelectNode(hitNode.properties!.id); cbs.onSelectEdge(null); cbs.onSelectBuilding(null); }
        else if (hitEdge) { cbs.onSelectEdge(hitEdge.properties!.id); cbs.onSelectNode(null); cbs.onSelectBuilding(null); }
        else if (hitBuilding) { cbs.onSelectBuilding(hitBuilding.properties!.id); cbs.onSelectNode(null); cbs.onSelectEdge(null); }
        else { cbs.onSelectNode(null); cbs.onSelectEdge(null); cbs.onSelectBuilding(null); }
        return;
      }
      if (curTool === "delete") {
      ...
      if (curTool === "add_node") {
        ...
        cbs.onAddNode({
          ...
          buildingId: nearBuilding?.id ?? undefined,
          floor: 1,
